target("example_material_editor")
    set_kind("binary")
    set_languages("cxx23")
    add_files("main.cpp")
    ...
    before_build(function (target) os.cp("$(scriptdir)/shaders", path.join(target:targetdir(), "shaders")) end)
    before_run(function (target) os.cp("$(scriptdir)/shaders", path.join(target:targetdir(), "shaders")) end)
    ...
    set_targetdir("$(builddir)/$(plat)/$(arch)/$(mode)/vshadersystem/examples/example_material_editor")
